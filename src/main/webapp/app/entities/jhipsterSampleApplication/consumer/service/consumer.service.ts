...
export class ConsumerService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/consumers', 'jhipstersampleapplication');
  ...
  protected convertDateFromClient(consumer: IConsumer): IConsumer {
    return Object.assign({}, consumer, {
      createdDate: consumer.createdDate?.isValid() ? consumer.createdDate.format(DATE_FORMAT) : undefined,
      updatedDate: consumer.updatedDate?.isValid() ? consumer.updatedDate.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((consumer: IConsumer) => {
        ...
        consumer.updatedDate = consumer.updatedDate ? dayjs(consumer.updatedDate) : undefined;
      });
    }
    return res;
  }
