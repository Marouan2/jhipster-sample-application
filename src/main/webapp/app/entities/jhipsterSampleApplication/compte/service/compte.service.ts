...
export class CompteService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/comptes', 'jhipstersampleapplication');
  ...
  protected convertDateFromClient(compte: ICompte): ICompte {
    return Object.assign({}, compte, {
      createdDate: compte.createdDate?.isValid() ? compte.createdDate.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((compte: ICompte) => {
        compte.createdDate = compte.createdDate ? dayjs(compte.createdDate) : undefined;
      });
    }
    return res;
  }
