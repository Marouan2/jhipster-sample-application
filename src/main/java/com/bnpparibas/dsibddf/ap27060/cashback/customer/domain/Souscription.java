package com.bnpparibas.dsibddf.ap27060.cashback.customer.domain;

import com.bnpparibas.dsibddf.ap27060.cashback.customer.domain.enumeration.StatusSouscription;
import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import java.io.Serializable;
import java.time.LocalDate;
import javax.persistence.*;
import org.hibernate.annotations.Cache;
import org.hibernate.annotations.CacheConcurrencyStrategy;

/**
 * A Souscription.
 */
@Entity
@Table(name = "souscription")
@Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
public class Souscription implements Serializable {

    private static final long serialVersionUID = 1L;

    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "sequenceGenerator")
    @SequenceGenerator(name = "sequenceGenerator")
    @Column(name = "id")
    private Long id;

    @Enumerated(EnumType.STRING)
    @Column(name = "status")
    private StatusSouscription status;

    @Column(name = "created_date")
    private LocalDate createdDate;

    @Column(name = "end_date")
    private LocalDate endDate;

    @ManyToOne
    @JsonIgnoreProperties(value = { "comptes" }, allowSetters = true)
    private Concent concent;

    // jhipster-needle-entity-add-field - JHipster will add fields here

    public Long getId() {
        return this.id;
    }

    public Souscription id(Long id) {
        this.setId(id);
        return this;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public StatusSouscription getStatus() {
        return this.status;
    }

    public Souscription status(StatusSouscription status) {
        this.setStatus(status);
        return this;
    }

    public void setStatus(StatusSouscription status) {
        this.status = status;
    }

    public LocalDate getCreatedDate() {
        return this.createdDate;
    }

    public Souscription createdDate(LocalDate createdDate) {
        this.setCreatedDate(createdDate);
        return this;
    }

    public void setCreatedDate(LocalDate createdDate) {
        this.createdDate = createdDate;
    }

    public LocalDate getEndDate() {
        return this.endDate;
    }

    public Souscription endDate(LocalDate endDate) {
        this.setEndDate(endDate);
        return this;
    }

    public void setEndDate(LocalDate endDate) {
        this.endDate = endDate;
    }

    public Concent getConcent() {
        return this.concent;
    }

    public void setConcent(Concent concent) {
        this.concent = concent;
    }

    public Souscription concent(Concent concent) {
        this.setConcent(concent);
        return this;
    }

    // jhipster-needle-entity-add-getters-setters - JHipster will add getters and setters here

    @Override
    public boolean equals(Object o) {
        if (this == o) {
            return true;
        }
        if (!(o instanceof Souscription)) {
            return false;
        }
        return id != null && id.equals(((Souscription) o).id);
    }

    @Override
    public int hashCode() {
        // see https://vladmihalcea.com/how-to-implement-equals-and-hashcode-using-the-jpa-entity-identifier/
        return getClass().hashCode();
    }

    // prettier-ignore
    @Override
    public String toString() {
        return "Souscription{" +
            "id=" + getId() +
            ", status='" + getStatus() + "'" +
            ", createdDate='" + getCreatedDate() + "'" +
            ", endDate='" + getEndDate() + "'" +
            "}";
    }
}
